...
pub fn load_special_texture(view: &mut EmbeddedTextureLoadView, material: &StandardMaterial) -> Option<GenericMaterial> {
		...
		&& view.name.starts_with("sky")
	{
		...
		fn separate_sky_image(view: &mut EmbeddedTextureLoadView, x_range: std::ops::Range<u32>, alpha_on_black: bool) -> Option<Image> {
			let pixel_size = view.image.texture_descriptor.format.pixel_size().ok()?;
